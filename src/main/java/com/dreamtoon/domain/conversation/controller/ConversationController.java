...
@Hidden
@Tag(name = "Conversations", description = "**대화형 AI 에이전트**와의 대화를 관리하는 API입니다. 대화 시작 → 메시지 주고받기 → (선택) 감정 선택·스트레스 평가 단계가 있습니다. "
...
public class ConversationController {
        ...
        @Operation(summary = "대화 시작", description = "**새 대화**를 시작합니다. 이미 진행 중인 대화가 있으면 그 대화 정보를 반환하고, 없으면 새로 생성 후 반환합니다. "
        ...
        @Operation(summary = "대화 조회", description = "`conversationId`에 해당하는 **대화 메타 정보**를 조회합니다. 대화 목록에서 한 건을 선택했을 때 상세 진입 전에 호출하면 됩니다.")
        ...
        @Operation(summary = "메시지 목록 조회", description = "해당 대화의 **모든 메시지**를 **시간순**으로 조회합니다. 채팅 화면 진입 시 과거 대화 내역을 불러올 때 사용하세요.")
        ...
        @Operation(summary = "메시지 전송", description = "사용자 메시지를 보내고 **AI 응답**을 바로 받습니다. 요청 body에 `content`(메시지 텍스트)를 넣어 보내면 "
        ...
        @Operation(summary = "감정 선택", description = "**감정 수집 단계**에서 사용합니다. 꿈에서 느낀 감정과 강도를 선택해 보내면, "
        ...
        public ResponseEntity<ApiResponse<EmotionSummaryResponse>> selectEmotions(
                ...
                EmotionSummaryResponse response = conversationService.selectEmotions(conversationId, request);
                return ResponseEntity.ok(ApiResponse.success("감정이 분석되었습니다.", response));
        }

        @Operation(summary = "스트레스 평가", description = "**상황 분석 단계**에서 사용합니다. 사용자의 현재 스트레스 수준/항목을 보내면 "
        ...
        public ResponseEntity<ApiResponse<StressAssessmentResponse>> submitStressAssessment(
                ...
                StressAssessmentResponse response = conversationService.submitStressAssessment(conversationId, request);
                return ResponseEntity.ok(ApiResponse.success("스트레스 평가가 완료되었습니다.", response));
        }

        @Operation(summary = "대화 삭제", description = "해당 대화와 **연결된 모든 메시지**를 삭제합니다. 본인이 소유한 대화만 삭제할 수 있으며, 삭제 후 복구할 수 없습니다.")
