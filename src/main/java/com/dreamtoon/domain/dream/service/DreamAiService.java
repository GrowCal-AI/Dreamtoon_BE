...
public class DreamAiService {
    ...
    public void generateWebtoonAsync(Long dreamId) {
        ...
        try {
            ...
            if (storyboardNode.has("characterDNA") && storyboardNode.has("scenes")) {
                ...
                sceneDescriptions = objectMapper.readValue(
            ...
            for (int panel = 1; panel <= WEBTOON_PANEL_COUNT; panel++) {
                ...
                CompletableFuture<String> future =
                        CompletableFuture.supplyAsync(
                                () -> generateSinglePanel(dreamId, panelNum, sceneDesc, finalCharDNA, dream),
                                imageGenerationPool);
                futures.add(future);
            }
    ...
    private String generateSinglePanel(
            ...
            String characterDNA, Dream dream) {
        int maxRetries = 3;
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                log.info("[PANEL {}] Attempt {}/{} via {} on thread: {}",
                        panelNum, attempt, maxRetries,
                        imageGenerationProvider.getProviderName(),
                        Thread.currentThread().getName());
                String prompt = DreamAnalysisPrompt.createWebtoonPanelPrompt(
                        currentScene, characterDNA, dream.getSelectedGenre(),
                        panelNum, WEBTOON_PANEL_COUNT);
    ...
    /**
     * content policy 거부 시 프롬프트를 완화.
     * 위험한 키워드를 제거하고 더 부드러운 표현으로 교체.
     */
    private String softenPromptForRetry(String originalPrompt) {
        return originalPrompt
                .replaceAll("(?i)(blood|gore|violent|weapon|knife|gun|sword|dead|death|kill)", "dramatic tension")
                .replaceAll("(?i)(horror|terrifying|scary|nightmare)", "mysterious and dreamlike")
                ...
                + "\nGentle, dreamy atmosphere. Safe for all audiences. Family-friendly illustration.";
    }
}
