...
public class DreamAiService {
        ...
        public void analyzeDreamAsync(Long dreamId) {
                ...
                try {
                        ...
                        log.info(
                                        "[ASYNC] Dream processing completed with fallback data for dream ID: {}",
                                        dreamId);
                }
        }
        ...
        public void analyzeDream(Dream dream) {
                try {
                        ...
                        DreamAnalysisResult result =
                                        analyzeDreamWithGpt(dream.getRawContent(), dream.getStylePreset());
                        ...
                        log.info(
                                        "AI dream analysis completed for dream ID: {}, {} scenes created",
                                        dream.getId(),
                                        scenes.size());
        ...
        private List<Scene> createScenesFromAnalysis(
                        List<DreamAnalysisResult.SceneDto> sceneDtos, Dream dream) {
        ...
        private String generateSceneImage(DreamAnalysisResult.SceneDto sceneDto, StylePreset style) {
                try {
                        String[] keywords =
                                        sceneDto.getBackgroundKeywords() != null
                                                        ? sceneDto.getBackgroundKeywords().toArray(new String[0])
                                                        : new String[] {};

                        String prompt =
                                        DreamAnalysisPrompt.createImagePrompt(
                                                        sceneDto.getDescription(), keywords, style);

                        // 1. DALL-E로 이미지 생성 (임시 URL, 60분 유효)
                        String tempDalleUrl = openAiClient.generateImage(prompt);
                        log.info(
                                        "Generated DALL-E image for scene {}: {}",
                                        sceneDto.getSceneNumber(),
                                        tempDalleUrl);

                        // 2. S3에 영구 저장
                        String permanentS3Url = s3StorageService.uploadImageFromUrl(tempDalleUrl, "scenes");
                        log.info(
                                        "Uploaded scene {} image to S3: {}", sceneDto.getSceneNumber(), permanentS3Url);
                        ...
                        // 실패 시 플레이스홀더 이미지
                        return "https://via.placeholder.com/1024x1792.png?text=Scene+"
                                        + sceneDto.getSceneNumber();
                }
        }

        /** 분석 결과로부터 Analysis 엔티티 생성 */
        private Analysis createAnalysisFromResult(
                        DreamAnalysisResult.AnalysisDto analysisDto, Dream dream) {
                // emotions의 Double을 그대로 사용 (0-100 범위)
                Map<com.dreamtoon.domain.dream.entity.EmotionType, Double> emotions =
                                analysisDto.getEmotions();
        ...
        private int calculateHealthScore(
                        ...
                        Boolean isNightmare) {
                double peace =
                                emotions.getOrDefault(com.dreamtoon.domain.dream.entity.EmotionType.PEACE, 0.0);
                double joy = emotions.getOrDefault(com.dreamtoon.domain.dream.entity.EmotionType.JOY, 0.0);
                double anxiety =
                                emotions.getOrDefault(com.dreamtoon.domain.dream.entity.EmotionType.ANXIETY, 0.0);
                double anger =
                                emotions.getOrDefault(com.dreamtoon.domain.dream.entity.EmotionType.ANGER, 0.0);
                double sadness =
                                emotions.getOrDefault(com.dreamtoon.domain.dream.entity.EmotionType.SADNESS, 0.0);
        ...
        private void createFallbackData(Dream dream) {
                ...
                Scene scene =
                                Scene.builder()
                                                ...
                                                .imageUrl(
                                                                "https://via.placeholder.com/1024x1792.png?text=AI+Processing+Failed")
                                                .build();
