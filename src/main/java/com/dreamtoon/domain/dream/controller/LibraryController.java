...
@Tag(
        name = "Library",
        description =
                "**라이브러리**는 사용자가 '라이브러리에 추가'한 꿈만 모아서 보는 공간입니다. "
                        + "즐겨찾기/장르/검색/정렬 필터를 지원하며, **Authorization: Bearer {accessToken}** 이 필요합니다.")
@RestController
...
public class LibraryController {
    ...
    @Operation(
            summary = "라이브러리 목록 조회",
            description =
                    "**저장한 꿈** 목록을 페이징·필터링하여 조회합니다. "
                            + "**쿼리 파라미터**: `favorite`(true=즐겨찾기만), `genre`(ROMANCE|FANTASY|HEALING|HORROR), "
                            + "`search`(제목/내용 검색어), `sort`(기본값 latest=최신순). "
                            + "페이지네이션은 `page`, `size` 로 지정하세요.")
    @GetMapping
    public ResponseEntity<ApiResponse<LibraryResponse>> getLibrary(
            @AuthenticationPrincipal Long userId,
            @Parameter(description = "true면 즐겨찾기한 꿈만") @RequestParam(required = false) Boolean favorite,
            ...
            @Parameter(description = "정렬: latest(최신순) 등") @RequestParam(required = false, defaultValue = "latest") String sort,
