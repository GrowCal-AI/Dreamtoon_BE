...
@Tag(name = "Dreams", description = "**꿈 기록 → 감정 선택 → 상세 입력 → AI 분석 → 4컷 웹툰 생성** 전체 플로우를 제공하는 API입니다. "
                + "모든 API는 **Authorization: Bearer {accessToken}** 헤더가 필요합니다. "
                + "호출 순서: 1) 꿈 기록 시작 → 2) 감정 선택 → 3) 상세 설명 입력 → 4) 분석 결과 조회(폴링) → 5) 웹툰 생성 → 6) 꿈 상세/라이브러리.")
@RestController
...
public class DreamController {
        ...
        @Operation(summary = "1. 꿈 기록 시작", description = "**[화면: 메인] 꿈 기록 시작**\n\n"
                        + "사용자가 꿈 내용을 입력하고 전송 버튼을 누르면 호출합니다. "
                        + "반환된 `dreamId`는 이후 감정 선택, 상세 입력, 결과 조회 등 모든 단계에서 사용되므로 프론트엔드에서 계속 가지고 있어야 합니다.")
        @PostMapping
        ...
        @Operation(summary = "2. 감정 선택", description = "**[화면: 감정 선택]**\n\n"
                        + "꿈 기록 시작 후, 사용자가 6가지 감정 칩 중 하나를 선택했을 때 호출합니다. "
                        + "요청 body에 감정 종류(`JOY`, `ANXIETY`, `ANGER`, `SADNESS`, `DISCOMFORT`, `CALM`)를 보내주세요.")
        ...
        @Operation(summary = "3. 상세 설명 입력", description = "**[화면: 상세 설명]**\n\n"
                        + "꿈에 대한 상세 내용과 현실 반영 질문(선택)을 입력하고 완료를 누르면 호출합니다. "
                        + "이 API가 호출되면 백엔드에서 **비동기 AI 분석**이 시작됩니다. "
                        + "응답으로 `202 Accepted`를 받으면 로딩 화면을 띄우고, **GET /{dreamId}/analysis** API를 폴링해주세요.")
        ...
        @Operation(summary = "4. 분석 결과 조회 (폴링용)", description = "**[화면: 로딩 중] 분석 상태 폴링**\n\n"
                        + "상세 설명 입력(`PATCH /details`) 후, AI 분석이 완료되었는지 확인하기 위해 **2~3초 간격**으로 계속 호출해야 합니다. "
                        + "응답의 `status`가 `ANALYSIS_COMPLETED`가 될 때까지 로딩을 유지하다가, 완료되면 결과 화면(장르 선택 전)을 보여주세요.")
        ...
        @Operation(summary = "5. 4컷 만화 생성", description = "**[화면: 장르 선택]**\n\n"
                        + "사용자가 웹툰 장르(`ROMANCE`, `FANTASY`, `HEALING`, `HORROR`)를 선택하면 호출합니다. "
                        + "호출 즉시 **비동기 이미지 생성**이 시작되며 `202 Accepted`가 반환됩니다. "
                        + "이후 다시 로딩 화면을 띄우고, **GET /{dreamId}** (꿈 상세) API를 폴링하거나 완료 후 호출하여 최종 결과를 확인하세요.")
        ...
        @Operation(summary = "꿈 상세 조회", description = "**[화면: 결과 확인 / 라이브러리 상세]**\n\n"
        ...
        @Operation(summary = "내 꿈 목록 조회", description = "**[화면: 마이페이지 / 내 꿈 기록]** (MVP 제외 가능)\n\n"
        ...
        @Operation(summary = "꿈 삭제", description = "**[화면: 상세 페이지 / 마이페이지] (선택 기능)**\n\n"
                        + "해당 꿈 기록을 영구 삭제합니다. "
        ...
        @Operation(summary = "라이브러리에 추가", description = "꿈을 **내 라이브러리**에 저장합니다. 저장된 꿈은 **GET /api/v1/library** 에서 필터/검색과 함께 조회할 수 있습니다. "
        ...
        @Operation(summary = "즐겨찾기 토글", description = "해당 꿈의 **즐겨찾기** 상태를 켜기/끄기 합니다. 한 번 호출할 때마다 on↔off가 바뀌며, "
                        + "응답의 `isFavorite`로 현재 상태를 반영하세요. 라이브러리 조회 시 `favorite=true` 로 필터링 가능합니다.")
        ...
        @Operation(summary = "꿈 더 대화하기 (채팅 전송)", description = "**[화면: 꿈 더 대화하기] 채팅 전송**\n\n"
        ...
        @Operation(summary = "채팅 내역 조회", description = "**[화면: 꿈 더 대화하기] 채팅 내역 로드**\n\n"
